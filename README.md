...
![Sentimento vs. volatilidade](outputs/04_sentiment_vs_volatility.png)
...
![Comparação out-of-sample](outputs/05_backtest_oos_comparison.png)
...
Ver `outputs/06_metrics_table.png` para a tabela completa de métricas.
...
![Backtest da estratégia](outputs/07_strategy_backtest.png)
 
O painel superior mostra o patrimônio acumulado das três estratégias; o painel inferior mostra a exposição ao Ibovespa variando no tempo (0.73x a 1.03x) — o modelo reduzindo posição quando a volatilidade prevista sobe.
 
### Conclusão honesta da estratégia
 
O período de teste foi de **queda do Ibovespa** (buy-and-hold perde -6.06% a.a.), então nenhuma das três estratégias teve retorno positivo — não maquiamos isso. Mas o vol-targeting **cumpriu o papel que se propõe**: reduziu volatilidade realizada (13.6-15.1% vs. 16.1% do buy-and-hold) e reduziu o drawdown máximo (-5.0%/-5.6% vs. -6.2%), amortecendo visivelmente a queda forte no fim da amostra. O Sharpe continua negativo nas três (o período inteiro teve retorno de mercado negativo, então nenhuma alocação linear no Ibovespa evitaria isso), mas é **menos negativo** nas duas versões com vol-targeting.
 
Entre as duas versões do vol-targeting, a que usa o **GARCH baseline supera a que usa o GARCH-X** em todas as métricas reportadas — maior retorno, menor vol., menor drawdown, Sharpe menos negativo.
 
**Ressalva, aplicando aqui o mesmo padrão do Passo 5:** essa comparação sofre exatamente da mesma limitação amostral que diagnosticamos na comparação estatística. São os mesmos 50 pregões, num único regime de mercado, e não aplicamos teste formal de diferença de Sharpe. A diferença entre -0.14% e -0.56% de retorno acumulado **não é estatisticamente distinguível de ruído**. A leitura correta é que o GARCH-X **não superou** o baseline, não que tenha sido confiavelmente pior. A direção é consistente com o resultado estatístico do Passo 4/5 — o que é sugestivo, e sugestivo não é conclusivo.
 
O que se pode afirmar com segurança: nesta amostra, o modelo mais simples não foi um adversário fácil de vencer, nem em termos estatísticos nem em termos de carteira.
...
- **Um único regime de mercado no teste.** Os 50 pregões de teste são de mercado em queda. A estratégia de vol-targeting não foi avaliada em alta sustentada, onde reduzir exposição custa retorno em vez de proteger.
## Próximos passos
 
Em ordem de retorno esperado sobre o esforço:
 
1. **Fonte de notícias com arquivo histórico real** (API paga ou base acadêmica), para levar a amostra a 500+ observações e tornar a inferência sobre γ estável. É a limitação que mais restringe qualquer conclusão deste trabalho.
2. **Filtro de relevância nas manchetes.** 88% da amostra é macro genérica; o sinal idiossincrático (corporativo, setorial, político) é o que teoricamente teria poder preditivo e está sub-representado. Filtrar ou ponderar por categoria antes de agregar pode revelar sinal que a média diária dilui.
3. **Reestimação em janela expansível no backtest**, eliminando a simplificação de parâmetros fixos e aproximando o teste de uma implementação real.
4. **Proxy de volatilidade realizada com dados intradiários** no lugar da janela móvel de 5 dias, reduzindo o ruído do benchmark contra o qual as previsões são avaliadas.
5. **Teste formal de diferença de Sharpe** (Jobson-Korkie com correção de Memmel) na comparação de estratégias, fechando a lacuna de rigor apontada na seção anterior.
...
│   └── strategy.py             # Fase 2: vol-targeting, 3 estratégias, métricas de portfólio
├── data/
