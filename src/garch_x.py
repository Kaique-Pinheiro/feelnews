...
def plot_comparison(df: pd.DataFrame, sigma2_baseline, sigma2_x, out_path: str) -> None:
    ...
    ax.plot(df.index, vol_baseline, label="GARCH(1,1) baseline (mesma janela)", color="#1f4e79", linewidth=1.1)
    ax.plot(df.index, vol_x, label="GARCH-X (sentimento defasado)", color="#c0392b", linewidth=1.1, linestyle="--")
